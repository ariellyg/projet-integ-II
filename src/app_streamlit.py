...
if st.button("Gerar Receita"):
    if not ingredientes_lista:
        ...
        with st.spinner("Gerando receita com IA..."):
            try:
                if validar_prompt_injection(ingredientes_lista):
                    st.error("⚠️ Ingredientes suspeitos de conter manipulação maliciosa. Corrija e tente novamente.")
                # elif not validar_input_contem_ingredientes(ingredientes_lista):
                #     st.error("⚠️ Ingredientes inválidos ou não reconhecidos. Por favor, verifique e tente novamente.")
                else:
